...
module Actors = 
    let chartingActor (chart : Chart) (pauseButton : System.Windows.Forms.Button) (mailbox : Actor<_>) = 
        ...
        let setChartBoundaries (mapping : Map<string, Series>, noOfPts : int) = 
            ...
            if allPoints
               ...
               > 2 then 
                let yValues = 
                    allPoints
                    |> Seq.collect (fun p -> p.YValues)
                    |> Seq.toList
                chart.ChartAreas.[0].AxisX.Maximum <- float noOfPts
                chart.ChartAreas.[0].AxisX.Minimum <- (float noOfPts - float maxPoints)
                chart.ChartAreas.[0].AxisY.Maximum <- if not (List.isEmpty yValues) then Math.Ceiling(yValues |> List.max)
                                                      else 1.
                chart.ChartAreas.[0].AxisY.Minimum <- if not (List.isEmpty yValues) then Math.Floor(yValues |> List.min)
                                                      else 0.
            else ()
        ...
        let rec charting (mapping : Map<string, Series>, noOfPts : int) = 
            actor { 
                ...
                | AddSeries series when not <| String.IsNullOrEmpty series.Name && mapping
                                                                                   |> Map.containsKey series.Name
                                                                                   |> not -> 
    ...
    let performanceCounterActor (seriesName : string) (performanceCounterGenerator : unit -> PerformanceCounter) (mailbox : Actor<_>) = 
        ...
        let cancelled = 
            mailbox.Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(TimeSpan.FromMilliseconds 250., TimeSpan.FromMilliseconds 250., mailbox.Self, GatherMetrics, ActorRefs.NoSender)
        mailbox.Defer(fun () -> 
            cancelled.Cancel()
            counter.Dispose() |> ignore)
        let rec loop (subscriptions) = 
            actor { 
                let! message = mailbox.Receive()
    ...
    let performanceCounterCoordinatorActor chartingActor (mailbox : Actor<_>) = 
        let counterGenerators = 
            Map.ofList [ CounterType.Cpu, fun () -> new PerformanceCounter("Processor", "% Processor Time", "_Total", true)
        ...
        let counterSeries = 
            Map.ofList [ CounterType.Cpu, fun () -> new Series(CounterType.Cpu.ToString(), ChartType = SeriesChartType.SplineArea, Color = Color.DarkGreen)
        ...
        let rec loop (counterActors : Map<CounterType, IActorRef>) = 
            actor { 
                let! msg = mailbox.Receive()
                match msg with
                | Watch counter when counterActors
                                     |> Map.containsKey counter
                                     |> not -> 
    ...
    let buttonToggleActor coordinatorActor (myButton : System.Windows.Forms.Button) myCounterType isToggled (mailbox : Actor<_>) = 
        let flipToggle (isOn) = 
            ...
            myButton.Text <- (sprintf "%s (%s)" ((myCounterType.ToString()).ToUpperInvariant()) (if isToggledOn then "ON"
                                                                                                 else "OFF"))
            isToggledOn
        
        let rec loop (isToggledOn) = 
            actor { 
                let! message = mailbox.Receive()
                match message with
